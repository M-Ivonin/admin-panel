...
describe('partner market config API', () => {
  ...
  it('uses the dedicated pause endpoint and trims its reason', async () => {
    ...
    expect(adminAuthFetch).toHaveBeenCalledWith({
      path: '/partner-market-configs/admin/config%2F1/pause',
      method: 'POST',
      body: JSON.stringify({ reason: 'Legal review withdrawn' }),
    });
  });
  ...
  it('surfaces backend validation messages', async () => {
    (adminAuthFetch as jest.Mock).mockResolvedValue({
      ...
      json: async () => ({
        message: [
          'countryCode must be longer than or equal to 2 characters',
          'countryCode must be shorter than or equal to 2 characters',
        ],
      }),
    });
    await expect(getPartnerMarketConfigs()).rejects.toThrow(
      'countryCode must be longer'
    );
  });
});
