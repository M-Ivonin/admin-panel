...
const terminalStates = new Set<EmailPublicationState>(['sent', 'completed_no_send', 'sent_with_failures', 'cancelled', 'failed', 'superseded']);
...
type Confirmation = { action: 'pause' | 'resume' | 'cancel' | 'sendNow'; title: string; confirmLabel: string };

export function EmailMarketingDashboard({ repository = emailMarketingRepository }: { repository?: EmailMarketingRepository }) {
  ...
  async function saveDraft() {
    ...
    try {
      ...
      const saved = selected
        ? await repository.edit(selected.id, { ...input, expectedDefinitionVersion: selected.definitionVersion })
        : await repository.create(input, createKey.current);
      await loadList();
      const detail = await repository.get(saved.id);
      setSelected(detail); setDraft(fromPublication(detail)); setEditorDirty(false); setPreview(null);
  ...
  async function estimateAudience() {
    if (!draft) return;
    setBusy(true); setError(null);
    try { setEstimate(await repository.estimateAudience(draft.audience)); }
    catch (caught) { setError(messageOf(caught)); }
    finally { setBusy(false); }
  }
  ...
  async function runCommand(action: Confirmation['action']) {
    ...
    try {
      ...
      const detail = await repository.get(selected.id);
      setSelected(detail); setDraft(fromPublication(detail)); setEditorDirty(false);
      await loadList(); setConfirmation(null); setCancelReason('');
    } catch (caught) { setError(messageOf(caught)); }
    finally { setBusy(false); }
  }
  ...
  async function approve() {
    if (!selected) return;
    setBusy(true); setError(null);
    try { await repository.approve(selected.id); await refreshDetail(); await loadList(); }
    catch (caught) { setError(messageOf(caught)); }
    finally { setBusy(false); }
  }
  ...
  async function schedule() {
    if (!selected) return;
    if (!scheduleLocal || !timezone.trim()) { setError('Schedule requires date, time, and an explicit IANA timezone.'); return; }
    setBusy(true); setError(null);
    try {
      await repository.schedule(selected.id, { scheduledAtUtc: zonedLocalDateTimeToUtc(scheduleLocal, timezone.trim()), timezone: timezone.trim() });
  ...
  return <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
    <AdminPageHeader title="Email Marketing" subtitle="Manual SirBro email publications with backend-authoritative approval and delivery outcomes." icon={<Email color="primary" />} actions={<Button variant="contained" startIcon={<Add />} onClick={() => void startCreate()}>Create publication</Button>} />
    <Box sx={{ maxWidth: 1440, mx: 'auto', p: { xs: 2, sm: 3, lg: 4 } }}>
      <Stack spacing={3}>
        <Alert severity="info">SirBro is fixed by the backend. Email eligibility, legal projections, lifecycle state, and counters are never inferred in this page.</Alert>
...
type EditorProps = {
  ...
  scheduleLocal: string; setScheduleLocal: (value: string) => void; timezone: string; setTimezone: (value: string) => void;
  onSchedule: () => void; onConfirm: (confirmation: Confirmation) => void;
};
...
function Editor(props: EditorProps) {
  ...
  const compatibleTemplates = sendGridTemplates.filter((item) => item.compatibleTopics.includes(draft.topic));
  const selectedTemplate = compatibleTemplates.find((item) => item.id === draft.sendGridTemplateId);
  const templateOptions = selectedTemplate || !draft.sendGridTemplateId ? compatibleTemplates : [{ id: draft.sendGridTemplateId, name: `Saved incompatible template (${draft.sendGridTemplateId})`, compatibleTopics: [], versions: [] }, ...compatibleTemplates];
  const activeVersions = selectedTemplate?.versions.filter((version) => version.active) ?? [];
  const selectedVersion = selectedTemplate?.versions.find((version) => version.id === draft.sendGridTemplateVersion);
  const versionOptions = selectedVersion && !selectedVersion.active ? [selectedVersion, ...activeVersions] : activeVersions.length ? activeVersions : draft.sendGridTemplateVersion ? [{ id: draft.sendGridTemplateVersion, name: `Saved version (${draft.sendGridTemplateVersion})`, active: false, updatedAt: '' }] : [];
  const set = <K extends keyof EditorDraft>(key: K, value: EditorDraft[K]) => updateDraft({ ...draft, [key]: value });
  const setManualAudience = (audience: CampaignAudienceDefinition) => set('audience', { ...audience, segmentSource: 'manual_rules', sourceSegmentId: null });
  const setTopic = (topic: EmailPublicationTopic) => updateDraft({ ...draft, topic, sendGridTemplateId: '', sendGridTemplateVersion: '', predictionKey: '', productCtaEnabled: false, productCtaUrl: '', productCtaLabels: blankLocalized(), partnerMarketConfigId: '', offerHeadlineByLocale: blankLocalized(), offerBodyByLocale: blankLocalized(), materialTermsByLocale: blankLocalized(), offerExpiresAt: '', destinationUrl: '' });
  ...
  return <Card><CardContent><Stack spacing={3}>
    ...
    <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}><TextField fullWidth select label="SendGrid template" value={draft.sendGridTemplateId} onChange={(event) => { const template = compatibleTemplates.find((item) => item.id === event.target.value); updateDraft({ ...draft, sendGridTemplateId: event.target.value, sendGridTemplateVersion: template?.versions.find((version) => version.active)?.id ?? '' }); }} required helperText={compatibleTemplates.length ? 'Only templates compatible with this publication type are shown.' : 'No compatible SendGrid templates are available.'}>{templateOptions.map((template) => <MenuItem key={template.id} value={template.id}>{template.name}</MenuItem>)}</TextField><TextField fullWidth select label="SendGrid template version" value={draft.sendGridTemplateVersion} onChange={(event) => set('sendGridTemplateVersion', event.target.value)} required disabled={!draft.sendGridTemplateId}>{versionOptions.map((version) => <MenuItem key={version.id} value={version.id} disabled={!version.active}>{version.name}{version.active ? ' · active' : ' · saved inactive'}</MenuItem>)}</TextField></Stack>
    <Typography variant="subtitle1">Audience</Typography>
    <TextField select label="Audience source" value={draft.audience.segmentSource} onChange={(event) => set('audience', { ...draft.audience, segmentSource: event.target.value as CampaignAudienceDefinition['segmentSource'], sourceSegmentId: null })}><MenuItem value="manual_rules">Manual rules</MenuItem><MenuItem value="template_segment">Template segment</MenuItem></TextField>
    {draft.audience.segmentSource === 'template_segment' ? <TextField select label="Template audience" value={draft.audience.sourceSegmentId ?? ''} onChange={(event) => {
    ...
    <Box><Typography variant="subtitle2">Retention stages</Typography><Stack direction="row" flexWrap="wrap" useFlexGap>{retentionStages.map((stage) => <FormControlLabel key={stage} control={<Checkbox checked={draft.audience.criteria.retentionStages.includes(stage)} onChange={(event) => setManualAudience({ ...draft.audience, criteria: { ...draft.audience.criteria, retentionStages: toggle(draft.audience.criteria.retentionStages, stage, event.target.checked) } })} />} label={stage} />)}</Stack></Box>
    <TextField label="Exact user IDs (comma-separated)" value={draft.audience.criteria.userIds.join(', ')} onChange={(event) => setManualAudience({ ...draft.audience, criteria: { ...draft.audience.criteria, userIds: splitValues(event.target.value) } })} />
    <Box><Typography variant="subtitle2">Recipient locales</Typography>{locales.map((locale) => <FormControlLabel key={locale} control={<Checkbox checked={draft.audience.criteria.locales.includes(locale)} onChange={(event) => setManualAudience({ ...draft.audience, criteria: { ...draft.audience.criteria, locales: toggle(draft.audience.criteria.locales, locale, event.target.checked) } })} />} label={locale} />)}</Box>
    <FormControlLabel control={<Checkbox checked={draft.audience.suppression.excludeUsersWithoutPushOpens} onChange={(event) => setManualAudience({ ...draft.audience, suppression: { excludeUsersWithoutPushOpens: event.target.checked } })} />} label="Narrow to users with push opens (explicit audience criterion only)" />
    ...
    {draft.topic === 'sirbro_predictions' ? <Stack spacing={2}><Alert severity="info">Only future predictions with Complete Full Analysis are selectable. Approval freezes the prediction, selected SendGrid template reference, and Full Analysis CTA.</Alert><TextField select label="Eligible prediction and version" value={draft.predictionKey} onChange={(event) => set('predictionKey', event.target.value)} required>{predictions.map((prediction) => <MenuItem key={`${prediction.id}:${prediction.analysisVersion}`} value={`${prediction.id}:${prediction.analysisVersion}`}>{prediction.teamsNames ?? prediction.id} · analysis v{prediction.analysisVersion}</MenuItem>)}</TextField></Stack> : null}
...
function LocalizedContentEditor({ draft, setDraft }: { draft: EditorDraft; setDraft: (draft: EditorDraft) => void }) {
  const update = (locale: CampaignLocale, field: keyof EmailContentByLocale[CampaignLocale], value: string) => setDraft({ ...draft, contentByLocale: { ...draft.contentByLocale, [locale]: { ...draft.contentByLocale[locale], [field]: value } } });
  return <Box><Typography variant="h6">Exact localized content</Typography><Stack spacing={3} sx={{ mt: 2 }}>{locales.map((locale) => { const content = draft.contentByLocale[locale]; return <Card variant="outlined" key={locale}><CardContent><Stack spacing={2}><Typography variant="subtitle1">{locale.toUpperCase()} <Chip size="small" label={isComplete(content) ? 'complete' : 'incomplete'} color={isComplete(content) ? 'success' : 'warning'} /></Typography><TextField label={`${locale} subject`} value={content.subject} onChange={(event) => update(locale, 'subject', event.target.value)} required /><TextField label={`${locale} preheader`} value={content.preheader} onChange={(event) => update(locale, 'preheader', event.target.value)} required /><TextField label={`${locale} HTML body`} value={content.htmlBody} onChange={(event) => update(locale, 'htmlBody', event.target.value)} multiline minRows={5} required /><TextField label={`${locale} text body`} value={content.textBody} onChange={(event) => update(locale, 'textBody', event.target.value)} multiline minRows={4} required /></Stack></CardContent></Card>; })}</Stack></Box>;
}

function LocalizedSimpleFields({ label, value, onChange, multiline = false }: { label: string; value: LocalizedString; onChange: (value: LocalizedString) => void; multiline?: boolean }) {
  return <Stack spacing={1}>{locales.map((locale) => <TextField key={locale} label={`${locale} ${label.toLowerCase()}`} value={value[locale]} onChange={(event) => onChange({ ...value, [locale]: event.target.value })} multiline={multiline} minRows={multiline ? 2 : undefined} required />)}</Stack>;
}

function PartnerProjection({ partner }: { partner: PartnerMarketProjection }) {
  return <Card variant="outlined"><CardContent><Typography variant="subtitle1">Backend legal/display projection (read-only)</Typography><Stack spacing={0.5}><Typography>{partner.operatorDisplayName} · {partner.minimumAge}+</Typography><Typography variant="body2">Logo: {partner.operatorLogoUrl ?? 'Missing — approval will fail closed'}</Typography>{locales.map((locale) => <Typography variant="body2" key={locale}>{locale.toUpperCase()} disclosure: {partner.affiliateDisclosureByLocale?.[locale] ?? 'Missing — approval will fail closed'}</Typography>)}<Typography variant="body2">Warning: {partner.requiredWarningText}</Typography><Typography variant="body2">Responsible gambling: {partner.responsibleGamblingUrl}</Typography></Stack></CardContent></Card>;
}

function LifecycleActions({ selected, canSend, scheduleLocal, setScheduleLocal, timezone, setTimezone, onSchedule, onConfirm }: { selected: EmailPublication; canSend: boolean; scheduleLocal: string; setScheduleLocal: (value: string) => void; timezone: string; setTimezone: (value: string) => void; onSchedule: () => void; onConfirm: (value: Confirmation) => void }) {
  return <><Divider /><Typography variant="h6">Approval and lifecycle</Typography>{!canSend && selected.state === 'draft' ? <Alert severity="info">Send now and schedule become available only after backend approval.</Alert> : null}{canSend ? <Stack spacing={2}><Button variant="contained" onClick={() => onConfirm({ action: 'sendNow', title: 'Send publication now?', confirmLabel: 'Confirm send now' })}>Send now</Button><Stack direction={{ xs: 'column', md: 'row' }} spacing={2}><TextField label="Schedule date and time" type="datetime-local" InputLabelProps={{ shrink: true }} value={scheduleLocal} onChange={(event) => setScheduleLocal(event.target.value)} /><TextField label="IANA timezone" value={timezone} onChange={(event) => setTimezone(event.target.value)} /><Button variant="outlined" onClick={onSchedule}>Schedule</Button></Stack></Stack> : null}{selected.state === 'scheduled' || selected.state === 'sending' ? <Button variant="outlined" onClick={() => onConfirm({ action: 'pause', title: 'Pause publication?', confirmLabel: 'Confirm pause' })}>Pause</Button> : null}{selected.state === 'paused' ? <Button variant="outlined" onClick={() => onConfirm({ action: 'resume', title: 'Resume publication?', confirmLabel: 'Confirm resume' })}>Resume</Button> : null}{!terminalStates.has(selected.state) ? <Button color="error" variant="outlined" onClick={() => onConfirm({ action: 'cancel', title: 'Cancel publication?', confirmLabel: 'Confirm cancel' })}>Cancel</Button> : null}</>;
}
...
function toInput(draft: EditorDraft, partners: PartnerMarketProjection[], templates: SendGridTemplateReference[]): EmailPublicationInput {
  ...
  const template = templates.find((item) => item.id === draft.sendGridTemplateId && item.compatibleTopics.includes(draft.topic));
  if (!template?.versions.some((version) => version.id === draft.sendGridTemplateVersion && version.active)) throw new Error('Select the active version of a current SendGrid template.');
  ...
  if (draft.topic === 'sirbro_product_updates') {
    input.productUpdate = draft.productCtaEnabled ? { cta: { url: draft.productCtaUrl.trim(), labelByLocale: trimLocalized(draft.productCtaLabels) } } : {};
    if (draft.productCtaEnabled && (!draft.productCtaUrl.trim() || locales.some((locale) => !input.productUpdate?.cta?.labelByLocale[locale]))) throw new Error('CTA URL and exact en/es/pt labels are required when CTA is enabled.');
  }
  if (draft.topic === 'betting_partner_offers') {
    const partner = partners.find((item) => item.id === draft.partnerMarketConfigId);
    if (!partner) throw new Error('Select an approved partner market configuration.');
    if (!draft.offerExpiresAt || !draft.destinationUrl.trim() || [draft.offerHeadlineByLocale, draft.offerBodyByLocale, draft.materialTermsByLocale].some((value) => locales.some((locale) => !value[locale].trim()))) throw new Error('Complete partner offer copy, material terms, expiry, and destination are required.');
    input.partnerOffer = { partnerMarketConfigId: partner.id, offerHeadlineByLocale: trimLocalized(draft.offerHeadlineByLocale), offerBodyByLocale: trimLocalized(draft.offerBodyByLocale), materialTermsByLocale: trimLocalized(draft.materialTermsByLocale), offerExpiresAt: new Date(draft.offerExpiresAt).toISOString(), destinationUrl: draft.destinationUrl.trim(), countryCode: partner.countryCode, regionCode: partner.regionCode ?? undefined };
  }
  return input;
}

function blankLocalized(): LocalizedString { return { en: '', es: '', pt: '' }; }
function blankContent(): EmailContentByLocale { return { en: { subject: '', preheader: '', htmlBody: '', textBody: '' }, es: { subject: '', preheader: '', htmlBody: '', textBody: '' }, pt: { subject: '', preheader: '', htmlBody: '', textBody: '' } }; }
function isComplete(value: EmailContentByLocale[CampaignLocale]): boolean { return [value.subject, value.preheader, value.htmlBody, value.textBody].every((item) => item.trim()); }
function toggle<T>(items: T[], value: T, checked: boolean): T[] { return checked ? Array.from(new Set([...items, value])) : items.filter((item) => item !== value); }
function splitValues(value: string): string[] { return value.split(',').map((item) => item.trim()).filter(Boolean); }
function topicLabel(topic: EmailPublicationTopic): string { return topics.find((item) => item.value === topic)?.label ?? topic; }
