...
function Editor(props: EditorProps) {
  ...
  return <Card><CardContent><Stack spacing={3}>
    ...
    <FormControlLabel control={<Checkbox checked={draft.audience.suppression.excludeUsersWithoutPushOpens} onChange={(event) => set('audience', { ...draft.audience, suppression: { excludeUsersWithoutPushOpens: event.target.checked } })} />} label="Narrow to users with push opens (explicit audience criterion only)" />
    ...
    {draft.topic === 'sirbro_predictions' ? <Stack spacing={2}><Alert severity="info">Only future predictions with Complete Full Analysis are selectable. Approval freezes the prediction, SendGrid template, and Full Analysis CTA.</Alert><TextField select label="Eligible prediction and version" value={draft.predictionKey} onChange={(event) => set('predictionKey', event.target.value)} required>{predictions.map((prediction) => <MenuItem key={`${prediction.id}:${prediction.analysisVersion}`} value={`${prediction.id}:${prediction.analysisVersion}`}>{prediction.teamsNames ?? prediction.id} · analysis v{prediction.analysisVersion}</MenuItem>)}</TextField></Stack> : null}
