...
function Editor(props: EditorProps) {
  ...
  return <Card><CardContent><Stack spacing={3}>
    ...
    <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}><TextField fullWidth select label="SendGrid template" value={draft.sendGridTemplateId} onChange={(event) => { const template = sendGridTemplates.find((item) => item.id === event.target.value); updateDraft({ ...draft, sendGridTemplateId: event.target.value, sendGridTemplateVersion: template?.versions.find((version) => version.active)?.id ?? '' }); }} required helperText={sendGridTemplates.length ? 'Select by name; the stable ID is frozen by the backend.' : 'No SendGrid templates are available.'}>{templateOptions.map((template) => <MenuItem key={template.id} value={template.id}>{template.name}</MenuItem>)}</TextField><TextField fullWidth select label="SendGrid template version" value={draft.sendGridTemplateVersion} onChange={(event) => set('sendGridTemplateVersion', event.target.value)} required disabled={!draft.sendGridTemplateId}>{versionOptions.map((version) => <MenuItem key={version.id} value={version.id} disabled={!version.active}>{version.name}{version.active ? ' · active' : ' · saved inactive'}</MenuItem>)}</TextField></Stack>
...
function toInput(draft: EditorDraft, partners: PartnerMarketProjection[], templates: SendGridTemplateReference[]): EmailPublicationInput {
  ...
  const template = templates.find((item) => item.id === draft.sendGridTemplateId);
